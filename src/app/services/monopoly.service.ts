...
export class MonopolyService {
    ...
    constructor(private ls: LogService) {
        ...
        this.cards = [
            ...
            new Chance(
                4,
                'Pechowy dzień',
                'Zobaczyłeś mema ze swoją osobą, udajesz się do więzienia w celu ukrycia.',
                false
            ),
            new Chance(
                5,
                'Reklamacja',
                'Kupiłeś zepsutą lodówkę Samsunga, udajesz się na pole Samsung w celu reklamacji.',
                false
            ),
            new Chance(
                6,
                'Gorączka piątkowej nocy',
                'Strasznie się upiłeś, a w Twoim portfelu brakuje 150$. Pamiętasz na co je wydałeś?',
                false
            ),
            ...
            new Chance(
                9,
                'Paliwko',
                'Podczas podróży zabrakło Ci paliwa, udaj się na stacje BP i zatankuj za 150$.',
                false
            ),
        ];
        this.activePlayer = 0;
    ...
    movePlayer() {
        if (this.active.roundsInJail == 0) {
            ...
            this.ls.logs.push(
                this.active.name +
                    ' wylosował kości o wartościach ' +
                    this.dice1 +
                    ' oraz ' +
                    this.dice2 +
                    ' , i przemieszcza się na pole ' +
                    this.fields[this.active.location].name
            );
            let y =
                this.fields[this.active.location].owner == null
                    ? 'nikogo'
                    : this.fields[this.active.location].owner.name;
        ...
        if (polenormalne) {
            if (this.fields[this.active.location].owner == null) {
                ...
                this.ls.logs.push(
                    this.active.name +
                        ' płaci ' +
                        this.fields[this.active.location].tax +
                        ' $ ' +
                        ' dla ' +
                        this.fields[this.active.location].owner.name
                );
    ...
    buyField() {
        let decision = confirm(
            '[' +
                ...
                this.fields[this.active.location].name +
                ' za ' +
                this.fields[this.active.location].cost +
                ' $ ' +
                '?'
        );
        if (decision) {
